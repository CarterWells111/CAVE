...
describe("official site content", () => {
  ...
  it("keeps the four consent principles and current six-page journey", () => {
    ...
    expect(experienceSteps).toEqual([
      ["01", "身体与安全知识", "通过有来源、非色情的内容认识身体差异、健康与同意边界；医学结构图可以选择不看。"],
      ["02", "过夜期待与在意", "整理期待和担心；一起过夜不代表任何事情必须发生。"],
      ["03", "行为地图与边界", "对不同靠近方式逐一留下此刻的感受；更具体的健康教育内容可以选择不看、不答。"],
      ["04", "自我反思", "回看答案、表达难度与让自己更安心的条件；这里不会生成分数或准备度结论。"],
      ["05", "预设沟通练习", "用不录音、不请求麦克风、也不使用 AI 的本机预设情境练习暂停、调整或停止。"],
      ["06", "我的沟通草稿", "编辑并保存想保留的表达；段落可以暂时删除和恢复，内容只保存在本机。"]
    ]);
    expect(demoFeatures).toEqual([
      ["六页引导旅程", "完成本机成年自我声明和前言后，从身体与安全知识开始，逐页整理期待、边界、感受与表达。"],
      ["身体与安全知识", "查看有来源、非色情的身体知识；医学结构图可以选择不看，持续不适或明显新变化时会提示咨询医疗专业人员。"],
      ["边界表达与不使用 AI 的预设练习", "在行为地图中逐项留下感受，再使用不录音、不请求麦克风、也不使用 AI 的本机预设对话，练习暂停、调整或停止。"],
      ["只保存在本机的回顾、沟通草稿与内界手记", "编辑并保存只留在本机的沟通草稿，也可按主题回顾；登录后可把关键事件和后来的变化写进内界手记，手记按同设备账号隔离。成年确认与称呼可随账号恢复。"]
    ]);
  });
  ...
  it("freezes every nested navigation item and content tuple", () => {
    ...
    expect(demoFeatures.every((feature) => Object.isFrozen(feature))).toBe(true);
  ...
  it("keeps the approved support FAQ and excludes forbidden exact phrases", () => {
    ...
    expect(
      JSON.stringify({ demoFeatures, experienceSteps, navItems, principles, privacyPoints, site, supportFaq })
    ).not.toMatch(/云同步|准备度分数|本机加密数据库/u);
  });
});
