...
test("hot-switches an already mounted press-effect consumer", async () => {
  let listener: ((enabled: boolean) => void) | undefined;
  ...
  render(
    <MotionPreferencesProvider>
      <Button label="继续" onPress={jest.fn()} />
    </MotionPreferencesProvider>
  );
  await act(async () => undefined);
  ...
  expect(button.props.style.opacity).toBe(0.82);
  fireEvent(button, "responderRelease", { nativeEvent: {}, persist: jest.fn() });
