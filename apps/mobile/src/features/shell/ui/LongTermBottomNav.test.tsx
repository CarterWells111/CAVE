...
test("renders four compact vector destinations with color and icon size as the only selected emphasis", () => {
  ...
  expect(screen.getByText("sparkles-outline")).toHaveProp("color", darkTheme.color.primary);
  expect(screen.getByText("sparkles-outline")).toHaveProp("size", 24);
  expect(screen.getByText("book-outline")).toHaveProp("color", darkTheme.color.textSecondary);
  expect(screen.getByText("book-outline")).toHaveProp("size", 20);
